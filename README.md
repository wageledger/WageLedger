...
## Deploying (Render, with a persistent disk)

This app stores everything in a local SQLite file, so it needs a host with
**persistent disk** — not Vercel (its functions have a read-only filesystem
with only an ephemeral `/tmp`, so a SQLite file there gets wiped between
requests). Render, Railway and Fly.io all support persistent disks; this
repo ships a `render.yaml` Blueprint for Render since it's the simplest of
the three to set up from a Git repo.

1. Push this folder to a GitHub repo.
2. In Render: **New +** → **Blueprint** → pick the repo. Render reads
   `render.yaml` and creates a web service on the **Starter** plan with a
   1GB disk mounted at `/var/data` (set `DATA_DIR=/var/data` so the SQLite
   file and session-secret file live on that disk, not in the app folder —
   the app folder itself is rebuilt from Git on every deploy and is not
   persistent).
   - The **Free** plan does *not* support persistent disks and spins down
     after 15 minutes idle — fine to try the deploy, not fine to actually
     use, since data written on a free instance disappears on the next
     spin-down/redeploy.
3. Render generates a random `SUPERADMIN_PASSWORD` and `SECRET_KEY` for you
   (see the Environment tab after the first deploy to read the generated
   password) — log in once and change it from the Users screen.
4. Railway or Fly.io work too if you'd rather use those: same idea (a
   `$PORT`-bound gunicorn process, a persistent volume, `DATA_DIR` pointed
   at that volume's mount path) — Render's Blueprint just automates it.
