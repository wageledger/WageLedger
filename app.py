...
# DATA_DIR holds the SQLite file and the session-secret file. Defaults to the
# app folder (unchanged local behaviour). On a host with a persistent disk
# (Render, Railway, Fly.io), set the DATA_DIR env var to that disk's mount
# path so the database survives restarts and redeploys — the app folder
# itself is rebuilt from Git on every deploy and is NOT persistent.
DATA_DIR = os.environ.get("DATA_DIR", BASE_DIR)
os.makedirs(DATA_DIR, exist_ok=True)
DB_PATH = os.path.join(DATA_DIR, "wage_ledger.db")
...
app.config.update(
    ...
    # Render/Railway/Fly all terminate TLS in front of the app and forward
    # plain HTTP to it, so Flask sees an http:// request even though the
    # browser used https://. Trust that one proxy hop so url_for(),
    # secure-cookie checks, etc. see the real scheme.
...
# Run once at import time so the schema/bootstrap admin exist whether the app
# is started with `python app.py` or served by a production WSGI server such
# as gunicorn (`gunicorn app:app`), which never executes the __main__ block.
init_db()
